...
module.exports = {
    name: 'undeafen',
    aliases: ['undeaf', 'addears'],
    description: 'Undeafen user by mentioning or providing user ID.',
        ...
        try {
            ...
            for (const member of users) {
                if (member.voice.channel) {
                    try {
                        await member.voice.setDeaf(false, 'Undeafened by command');
                        const embed = new EmbedBuilder()
                            .setColor('#2ECC70')
                            .setDescription(`<:granted:1247140301060046907> Successfully undeafened ${member.user}.`);
                        message.reply({ embeds: [embed] });
                    } catch (error) {
                        const embed = new EmbedBuilder()
                            .setColor('#F39C12')
                            .setDescription(`<:error:1247140297784426567> An error occurred while undeafening ${member.user}.`);
                        message.reply({ embeds: [embed] });
                    }
                } else {
                    const embed = new EmbedBuilder()
